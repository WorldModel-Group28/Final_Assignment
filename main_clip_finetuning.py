...
def load_clip_model():
    ...
    # ファインチューニングされたモデルの重みをロード
    finetuned_model_path = '/home/kako/Final_Assignment-2/finetuned_clip_model.pth'
...
def get_button_num(problem, button_list, image_path):
    ...
    # ファインチューニングされたCLIPモデルと前処理ツールをロード
    ...
    if not match:
        ...
        return None
    item_to_count = match.group(1)
    print("抽出された文言:", item_to_count)

    # 確率算出するテキストを作成
    text_descriptions = [f"{button} {item_to_count}" for button in button_list]
    ...
    print(f"最も類似度の高い説明: {text_descriptions[top_choice_index]} (類似度: {probs[top_choice_index]:.4f})")
    
    # 一致する選択肢を返す
    return top_choice_index + 1  # インデックスを選択肢の番号に変換
